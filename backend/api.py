...
def Return_New_User():
    if request.method == "POST":
        ...
        user = models.User.query.filter_by(
            email=email
        ).first()  # looks in databse and tries to get the first occurence of this name in it
        if user is None:
...
def Login():
    ...
    if request.method == "POST":
        ...
        user = models.User.query.filter_by(
            email=email
        ).first()  # looks in databse and tries to get the first occurence of this email in it
...
def Return_New_Clothing_Item():
    try:
        ...
        if delete_item == "false":  # when user wants to add item to the closet
            ...
            clothing_item.img = item_dict["img"]
            # Construct Tag objects from the request (called attributes there),
            # And add them to the clothing_item
            tag_objects = [
                models.Tag.query.get(tag_name)
                if models.Tag.query.get(tag_name)
                else models.Tag(name=tag_name)
                for tag_name in item_dict["attributes"]
            ]
            ...
            print(clothing_item)
            exists = models.ClothingItem.query.filter_by(
                id=clothing_item.id
            ).first()
            if exists:
        ...
        if delete_item == "true":  # when user wants to delete item in closet
            ...
            for i in range(len(closet_list)):
                if (
                    closet_list[i].closet_id == item_dict["closet_id"]
                    and closet_list[i].name == item_dict["name"]
                ):
                    result = False
                    item_names = []
        ...
        tag_objects = set(
            [
                models.Tag.get_or_create(tag_name)
                for tag_name in item_dict["attributes"]
            ]
        )
...
def generate_outfit():
    ...
    outfit_template_id = helpers.get_default_template_id_from_weather_str(
        weather_str, temp
    )
    ...
    return [
        item.serialize_with_template_id(item_to_its_template[item])
        for item in items
    ]
...
def get_item_from_template():
    ...
    if "excluded_item" in data:
        excluded_items.add(
            models.ClothingItem.query.get(data["excluded_item"])
        )
    match: models.ClothingItem = user.default_closet().find_matching_item(
        item_template=item_template, excluded_items=excluded_items
    )
