...
export const WeatherDashboard = (props) => {
  ...
  const handleChange = (v) => {
    ...
    tempType == "f"
      ? setTemp(changeTemp(weather.temp0, tempType))
      : setTemp(changeTemp(weather.temp0, tempType));
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          {console.log(weather)}
          <Typography variant="h3" fontFamily="Caudex">
            {" "}
            Today's Weather for <b>{weather.city_name}</b>
          </Typography>
          <DigitalClock />
          <img
            src={weatherImg}
            style={{ float: "left", width: "45%", padding: "5%" }}
          />
          <div style={{ float: "right", padding: "5%" }}>
            {!isNaN(weather.temp0) && (
              <>
                <Typography variant="h4" fontFamily="Caudex">
                  <b>{temp}º </b>
                </Typography>
                <FormControl>
                  <RadioGroup
                  ...
                  >
                    <FormControlLabel
                      ...
                      control={<Radio />}
                      label={
                        <Typography fontFamily="Caudex">Fahrenheit</Typography>
                      }
                    />
                    <FormControlLabel
                      value="f"
                      control={<Radio />}
                      label={
                        <Typography fontFamily="Caudex"> Celcius </Typography>
                      }
                    />
                  </RadioGroup>
                </FormControl>
                <Typography variant="h6" fontFamily="Caudex">
                  {" "}
                  Today's Forecast Is: <b>{weather.weather0}</b>{" "}
                </Typography>
              </>
            )}
          </div>
