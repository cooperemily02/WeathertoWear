...
export const HomeDashboard = (props) => {
  ...
  const [fetchedOutfitData, setFetchedOutfitData] = useState({
    hasOutfit: false,
    fetchError: false,
  });
      ...
      <div>
        ...
        {zipcode !== -1 && fetchedOutfitData.hasOutfit == true && (
          <Outfit
            userId={userId}
            outfit={outfit}
            fetchedOutfitData={fetchedOutfitData}
            generateOutfitFunction={fetchGeneratedOutfit}
            updateSinglePiece={updateSinglePiece}
          />
        )}
        {fetchedOutfitData.hasOutfit == false &&
          fetchedOutfitData.fetchError == true && (
            <div>
              <Typography
                variant="h5"
                textAlign={"center"}
                sx={{ color: "black", fontFamily: "Caudex" }}
              >
                {" "}
                Unable to fetch an outfit at the current time. Please try again
                later or submit a trouble ticket.{" "}
              </Typography>
            </div>
          )}
      </div>
